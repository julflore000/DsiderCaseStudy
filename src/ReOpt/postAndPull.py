...
def get_api_results(post, API_KEY, api_url, results_file='results.json', run_id=None):
    ...
    if run_id is not None:
        ...
        if "dev/" in results_url:
            results = ReOpt.resultsPoller.poller_v3(url=results_url.replace('<run_uuid>', run_id))
        else:
            results = ReOpt.resultsPoller.poller(url=results_url.replace('<run_uuid>', run_id))

        with open(results_file, 'w') as fp:
            json.dump(obj=results, fp=fp)

        ReOpt.logger.log.info("Saved results to {}".format(results_file))
    else:
        results = None
        ReOpt.logger.log.error("Unable to get results: no run_uuid from POST.")
...
def get_run_uuid(post, API_KEY, api_url):
    ...
    run_id = None
    if not resp.ok:
        ReOpt.logger.log.error("Status code {}. {}".format(resp.status_code, resp.content))
    else:
        ReOpt.logger.log.info("Response OK from {}.".format(post_url))
